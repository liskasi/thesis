...
def compute_chunk(chunk, user_item_matrix, num_users, num_neighbors):
    ...
    for i in chunk:
        ...
        u1_items_set = set(observed_purchases_idx)
        ...
        for j in common_users_idx:
            if i != j:
                u2_items_set = set(user_item_matrix[j].indices)
                intersection = len(u1_items_set.intersection(u2_items_set))
                union = len(u1_items_set.union(u2_items_set))
...
def append_line_to_file(line):
    with open('jaccardNeighbourhood3.txt', 'a') as file:  # 'a' opens the file in append mode
