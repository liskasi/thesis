    ...
    for i in chunk:
        ...
        for j in common_users:
            if i != j:
                ...
                u1_mean = np.mean(u1_values)
                u2_mean = np.mean(u2_values)


                numerator = sum([(user_item_matrix[i,x] - u1_mean) * (user_item_matrix[j,x] - u2_mean) for x in common_indices])
                denominator_u1 = sum([(user_item_matrix[i,x]-u1_mean)**2 for x in common_indices])
                denominator_u2 = sum([(user_item_matrix[j,x]-u2_mean)**2 for x in common_indices])
                denominator = denominator_u1**0.5 * denominator_u2**0.5
